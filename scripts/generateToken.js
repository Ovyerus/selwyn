...
const asyncBytes = s => new Promise((resolve, reject) =>
    randomBytes(s, (err, buf) =>
...
async function main() {
    if (!hasEnvVars) {
        console.log(
            c.red(c.bold('It looks like you are missing the environment variables required for the script to run.'))
        );
        console.log(
            c.red(c.bold('Try adding them in a `.env` file, or adding them directly to your environment variables.'))
        );
        console.log(
            c.red(c.bold('For information on the required environment variables, refer to the env section of `now.json`.'))
        )
        process.exit(0);
    }
    ...
    if (alreadyGenerated) {
        const { confirmExists } = await prompt({
            type: 'confirm',
            name: 'confirmExists',
            message: 'It looks like you have already generated a passphrase. Are you sure you want to regenerate it?'
        });
    ...
    if (alreadyGenerated)
        await query`UPDATE redirect_keys SET content = ${ hash }`;
    else
        await query`INSERT INTO redirect_keys (content) VALUES (${ hash })`;
